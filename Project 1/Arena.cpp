...
bool Arena::attackZombieAt(int r, int c, int dir)
{
    ...
    {
        // since we know for sure that the zombie is going to be killed, we first record the present position of the player by dereferencing m_player and calling the row() and col() functions.
        int player_row = m_player->row();
        int player_col = m_player->col();
        // next, we use the recorded values of the present position of the player as parameters for the record member function of m_history, which will lead to the number of zombies killed by the player at this particular position being incremented by 1.
        m_history.record(player_row, player_col);
        // the zombie can now be killed.
        delete m_zombies[k];
...
// accessor function which returns a reference to a History object, which is m_history member variable of Arena class. This will be utilized by Game class at the time of calling History of the board (that is when 'h' is entered into the console).
